...
class AppFile extends FileManagerAppModel {
	...
	/**
	 * An array of video types we accept to the file plugin.
	 */
	...
	/**
	 * An array of audio types we accept to the file_manager plugin.
	 */
	...
	public $hasMany = array(
		'FileAttachment' => array(
			'className' => 'FileManager.FileAttachment',
			'foreignKey' => 'file_id_id'
			)
		);
	...
	public function __construct($id = false, $table = null, $ds = null) {
		...
		$this->themeDirectory = ROOT . DS . SITE_DIR . DS . 'Locale' . DS . 'View' . DS . WEBROOT_DIR . DS . 'files';
		...
		$this->fileUrl = '/theme/default/file_manager/';
	 ...
	 * If trying to upload & save a File object, pass it to this function.
	 *
	 * @param array $file
	 * @return boolean
	 */
		...
		for ($i=0; $i < count(array_values($data['Myfile'])); $i++) {
			if ((is_array($data['Myfile'][$i]['filename']) && !empty($data['Myfile'][$i]['filename']['name'])) || (!empty($data['Myfile'][$i]['filename']) && is_string($data['Myfile'][$i]['filename']))) {
				...
				if ($this->beforeUpload()) {
					...
					throw new Exception(__('File before upload failed'));
				}
			}
 ...
 */
	public function afterUpload($data) {
		$fileType = $this->fileType($this->fileExtension);
...
/**
 * Given a File object, this will take a snapshot of the video, save it in
 * /file_manager/images/, and record it's relative URL in File.thumbnail
 *
 * @todo Need to find a good way to get $options into here.  Perhaps Site
 * Settings? ^JB
 *
 * @param array $data File object
 * @param array $options Defaults are: array('thumbnailSize' => '150x150',
 * 'fromTime' => '00:00:5')
 * @return array|boolean Result from File->save()
 * @throws Exception
 */
	public function setVideoThumbnail($data, $options = array()) {
		...
		$uploadFile = $this->getFileFilePath($data);
...
/**
 * Gets and returns the mime type of the file
 *
 * @param $ext
 ...
 */
		...
		if (!empty($data['Myfile']['type']) && $data['Myfile']['type'] == 'record') {
			...
			if (file_exists($serverFile)) {
				if (rename($serverFile, $localFile)) {
					#echo $url =
					# '/theme/default/file_manager/'.$this->pluginFolder.'/videos/'.$fileName.'.flv';
				} else {
					return false;
				}
			} else {
	...
	private function _saveCanvasImageObject($data, $galleryId = false) {
		...
		if ($written) {
			...
			// );
			// $this->FileAttachment->save($fileAttachment);
			// }
		}
		return $added;
	}
