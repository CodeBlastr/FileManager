 ...
 */
	public function generateThumbnail($image, $options) {
		...
		if (!empty($options['model'])) {
			...
			$options['src'] = sprintf('%s%s/%s/%s/%s', WWW_ROOT, $meioupload_path, strtolower($options['model']), $options['field'], $image);
			unset($options['model']);
			unset($options['field']);
		} else {
 ...
 */
	public function thumbnail($image, $options, $htmlOptions = array()) {
		...
		return ROOT . DS . SITE_DIR . DS . 'Locale' . DS . 'View' . DS . 'webroot' . DS . 'files' . DS . $thumbnail['src'];
